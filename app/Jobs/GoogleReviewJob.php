...
class GoogleReviewJob implements ShouldQueue
{
        ...
        try{
            ...
            if ($booking?->bookingContact?->email) {
                Mail::to($booking?->bookingContact?->email)
                    ->send(new GoogleReviewMail($booking));

                $bookingEmail               = new BookingEmails();
                $bookingEmail->booking_id   = $booking->id;
                $bookingEmail->email        = $booking->bookingContact->email;
                $bookingEmail->subject      = 'Google Review';
                $bookingEmail->sent_by      = $this->sentBy;
                $bookingEmail->save();
            } else {
                Log::warning("GoogleReviewJob: No email found for booking ID {$this->bookingId}");
            }
        }catch(\Exception $e){
            Log::info("GoogleReviewJob Failed: ", $e->getMessage());
        }
    }
}
