...
use App\Mail\BookingCancelledByVendorMail;
...
class BookingCancelledByVendorJob implements ShouldQueue
{
        ...
        try{
            ...
            if (!$booking?->bookingContact?->email) {
                Mail::to($booking?->bookingContact?->email)
                    ->send(new BookingCancelledByVendorMail($booking));

                $bookingEmail               = new BookingEmails();
                $bookingEmail->booking_id   = $booking->id;
                $bookingEmail->email        = $booking->bookingContact->email;
                $bookingEmail->subject      = 'Booking Cancelled By Vendor';
                $bookingEmail->sent_by      = $this->sentBy;
                $bookingEmail->save();
            } else {
                Log::warning("BookingCancelledByVendorJob: No email found for booking ID {$this->bookingId}");
            }
        }catch(\Exception $e){
            Log::info("BookingCancelledByVendorJob Failed: ", $e->getMessage());
        }
    }
}
