...
class SearchRepository
{
    ...
    public function searchResult($request, $city)
    {
        try {
            ...
            $filteredHotels = $hotelsQuery->get()->map(function ($hotel) use ($nights, $city, $request, $searchData) {

                $availableRoom = $hotel->rooms->first();

                return $this->buildHotelResult($hotel, $availableRoom, $nights, $city, $request, $searchData);
            });
    ...
    private function buildHotelResult($hotel, $availableRoom, $nights, $city, $request, $searchData)
    {
        ...
        $totalGuests = $searchData->adultCount ?? 0 + $searchData->childCount ?? 0;
        $stayGuest = $availableRoom->stay_guest ?? 0;
        $totalRoom =  $searchData->roomCount ?? 0;

        $extraPersonCharge = calculateExtraPersonPrice($availableRoom->ratePlan, $totalGuests, $stayGuest, $totalRoom,'ep');
